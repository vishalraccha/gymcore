...
import { getActiveOwnerSubscription, getOwnerSubscriptionHistory } from '@/lib/ownerSubscriptions';
...
export default function GymProfileScreen() {
  ...
  const { profile, refreshProfile, refreshGym, signOut, gym } = useAuth();
  ...
  const [showOwnerSubscriptionModal, setShowOwnerSubscriptionModal] = useState(false);
  ...
  // Auto-refresh subscription when component mounts or refreshes
  useEffect(() => {
  ...
  const loadRecentPayments = async () => {
    if (!profile || !gym || profile.role !== 'gym_owner') return;
    
    try {
      const { data: history, error } = await getOwnerSubscriptionHistory(profile.id, gym.id);
      if (history && !error) {
        // Get recent payments (last 3)
        const recent = (Array.isArray(history) ? history : [])
          .filter((sub: any) => sub.payment_status === 'success')
          .slice(0, 3)
          .map((sub: any) => ({
            id: sub.id,
    ...
    <CheckCircle size={16} color="#FFFFFF" />
      ...
      {profile?.role === 'gym_owner' && (
        <OwnerSubscriptionModal
          visible={showOwnerSubscriptionModal}
          onClose={() => setShowOwnerSubscriptionModal(false)}
          onSuccess={async () => {
