...
// Lazy load charts - only loads when charts are displayed
import { LazyLineChart, LazyBarChart, LazyPieChart } from '@/components/lazy/LazyCharts';
import {
  Users,
  UserCheck,
  UserX,
...
export default function AdminDashboardScreen() {
  ...
  const [revenueFilter, setRevenueFilter] = useState<'day' | 'month' | 'total'>('month');
  ...
  const calculateRevenueTrend = (payments: any[], filter: 'day' | 'month' | 'total') => {
    if (filter === 'day') {
      // Last 7 days
      ...
      const last12Months: RevenueData[] = [];
      for (let i = 11; i >= 0; i--) {
        const date = new Date();
        date.setMonth(date.getMonth() - i);
        const monthStart = new Date(date.getFullYear(), date.getMonth(), 1);
...
amount: sub.paid_amount || 0,
      ...
      // Revenue trend data with filter
      const revenueTrend = calculateRevenueTrend(paymentsForTrend, revenueFilter);
        ...
        <Card style={styles.chartCard}>
          <View style={styles.filterContainer}>
            <TouchableOpacity
              style={[
                styles.filterButton,
                revenueFilter === 'day' && styles.filterButtonActive,
              ]}
              onPress={() => setRevenueFilter('day')}
              activeOpacity={0.7}
            >
              <Text style={[
                styles.filterButtonText,
                revenueFilter === 'day' && styles.filterButtonTextActive,
              ]}>
                Daily
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={[
                styles.filterButton,
                revenueFilter === 'month' && styles.filterButtonActive,
              ]}
              onPress={() => setRevenueFilter('month')}
              activeOpacity={0.7}
            >
              <Text style={[
                styles.filterButtonText,
                revenueFilter === 'month' && styles.filterButtonTextActive,
              ]}>
                Monthly
              </Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity
              style={[
                styles.filterButton,
                revenueFilter === 'total' && styles.filterButtonActive,
              ]}
              onPress={() => setRevenueFilter('total')}
              activeOpacity={0.7}
            >
              <Text style={[
                styles.filterButtonText,
                revenueFilter === 'total' && styles.filterButtonTextActive,
              ]}>
                Total
              </Text>
            </TouchableOpacity>
                    ...
                    <View key={index} style={styles.legendItem}>
                      <View style={styles.legendLeft}>
                        ...
                        <View style={styles.legendTextContainer}>
                          ...
                          <Text style={[styles.legendCount, { color: theme.colors.textSecondary }]}>
                            {item.population} members
                          </Text>
                        </View>
                      </View>
